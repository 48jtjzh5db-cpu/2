...
function renderStatus() {
    ...
    // 計算最高等級屬性的稱號
    const rankIndex = Math.min(Math.floor(maxLevel / 10), 10);
    const highestRank = (data.customRanks && data.customRanks[maxLevelAttr] && data.customRanks[maxLevelAttr][rankIndex]) || (data.customRanks && data.customRanks[maxLevelAttr] && data.customRanks[maxLevelAttr][0]) || "等級" + maxLevel;

    document.getElementById('total-lv').textContent = highestRank;
...
function renderSetting() {
    ...
    Object.keys(ATTR_MAP).forEach(attr => {
        ...
        for (let i = 0; i < 11; i++) {
            const level = i * 10 + (i === 0 ? 1 : 10);
            const rankName = ranks[i] || `等級${level}`;
            attrDiv.innerHTML += `
                <div class="rank-item" style="display:flex; flex-direction:column; gap:4px;">
                    <label style="font-size:0.7rem; color:#666;">Lv.${level}${i < 10 ? `-${level+9}` : '+'}</label>
...
function updateRank(attr, index, value) {
    ...
    data.customRanks[attr][index] = value.trim() || `等級${index * 10 + (index === 0 ? 1 : 10)}`;
    save();
    // 更新狀態頁面以顯示新的稱號
    renderStatus();
}
